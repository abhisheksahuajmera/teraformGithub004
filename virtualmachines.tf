...
//--
resource "azurerm_network_interface" "rg103vn100ni101tg" {
...
resource "azurerm_windows_virtual_machine" "r103100100101tg" {
  ...
  location            = local.location 
  size                = "Standard_D2s_v3"
  admin_username      = "r103100100100101"
  admin_password      = "Azure@123"      
    network_interface_ids = [
    azurerm_network_interface.rg103vn100ni101tg[each.key].id,
  ]
  ...
  depends_on = [
    azurerm_virtual_network.rg103vn100tg,
    azurerm_network_interface.rg103vn100ni101tg
  ]
}